...
class ADFormerHybrid(nn.Module):
    ...
    def forward(self, patches, features):
        ...
        """
        # Encode les features
        f = self.feature_encoder(features)  # (B, d_model)

        # Embed + position
        p = self.embed_patch(patches) + self.pos_embed

        # Token fusion : injection des features dans chaque patch
...
def train_loso(seed, save_path, h5_file):
    ...
    # === Modèle
    model = ADFormerHybrid().to(device)  # adapte à ton besoin binaire ou multi
...
#            loss = loss_fn(torch.log(fusion + eps), y)
            opt.zero_grad()
            loss.backward()
            opt.step()
...
#            torch.save(model.state_dict(), save_path)
            ...
            print(f"💾 Scaler sauvegardé : {scaler_name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save', type=str, default='adformer_model_C_scaler.pth',
    ...
    h5_file = os.path.join(data_dir, "eeg_data_alzheimer_99_id_C_scaler.h5")
